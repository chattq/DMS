...
export const MstPlant = () => {
  const { t } = useI18n("MstPlant");
  ...
  const onCreate = async (data: Partial<Mst_Plant>) => {
    const resp = await api.Mst_Plant_Create({
      ...data,
  ...
  const handleDeleteRows = async (rows: string[]) => {
    const resp = await api.Mst_Plant_Delete_Multiple(rows);
    if (resp.isSuccess) {
      toast.success(t("Delete Successfully"));
      await refetch();
    } else {
  ...
  const handleUploadFile = async (file: File, progressCallback?: Function) => {
    const resp = await api.Mst_Plant_Upload(file);
    if (resp.isSuccess) {
